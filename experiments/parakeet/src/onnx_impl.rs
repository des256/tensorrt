use {
    super::*,
    onnx::TensorElement,
    std::{any::TypeId, marker::PhantomData, sync::Arc, time::Instant},
};
...
pub struct Parakeet<T: TensorElement + Default> {
    onnx: Arc<onnx::Onnx>,
...
impl<T: TensorElement + Default + ToFromF32> Parakeet<T> {
    pub fn new(model_folder_path: &str, executor: onnx::Executor) -> Self {
        ...
        let decoder_state1 = onnx::Value::zeros::<T>(&onnx, &[2, 1, DECODER_STATE_DIM as i64]);
        let decoder_state2 = onnx::Value::zeros::<T>(&onnx, &[2, 1, DECODER_STATE_DIM as i64]);
        Self {
            onnx,
            feature_extractor,
            encoder_session,
    ...
    pub fn transcribe_window(&mut self, window: &[i16]) -> String {
        ...
        self.decoder_state1 =
            onnx::Value::zeros::<T>(&self.onnx, &[2, 1, DECODER_STATE_DIM as i64]);
        self.decoder_state2 =
            onnx::Value::zeros::<T>(&self.onnx, &[2, 1, DECODER_STATE_DIM as i64]);
        ...
        let mut transposed = vec![T::default(); MEL_SIZE * num_frames];
        for frame in 0..num_frames {
            for bin in 0..MEL_SIZE {
                transposed[bin * num_frames + frame] =
                    T::from_f32(features[frame * MEL_SIZE + bin]);
            }
        }

        let audio_signal_tensor =
            onnx::Value::from_slice(&self.onnx, &[1, MEL_SIZE, num_frames], &transposed);
        ...
        let enc_output = outputs[0].extract_tensor::<T>();
        let t_enc = enc_output.len() / ENCODER_OUTPUT_DIM;
        let mut encoded = vec![T::default(); t_enc * ENCODER_OUTPUT_DIM];
        ...
        while time_index < num_frames {
            ...
            let state1_data = self.decoder_state1.extract_tensor::<T>().to_vec();
            let state2_data = self.decoder_state2.extract_tensor::<T>().to_vec();
            ...
            if predicted_token == BLANK_ID {
                self.decoder_state1 = onnx::Value::from_slice::<T>(
                    &self.onnx,
                    &[2, 1, DECODER_STATE_DIM],
                    &state1_data,
                );
                self.decoder_state2 = onnx::Value::from_slice::<T>(
                    &self.onnx,
                    &[2, 1, DECODER_STATE_DIM],
                    &state2_data,
                );
